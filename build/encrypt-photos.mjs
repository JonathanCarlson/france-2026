// Encrypts every file in build/photos/ → data/photos/<name>.enc so the app can
// decrypt + display trip photos in-browser (offline, private). Same binary
// format as encrypt-assets.mjs: [16-byte salt][12-byte iv][AES-GCM ciphertext].
...
catch { console.log('ℹ️ No build/photos/ folder — no photos to encrypt.'); process.exit(0); }
...
for (const f of files) {
  ...
  console.log(`✅ data/photos/${name}.enc (${(out.length / 1024).toFixed(0)} KB)`);
}
console.log(`Done — ${files.length} photo(s) encrypted.`);
